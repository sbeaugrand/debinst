...
if len(sys.argv) <= 8:
    print(f'Usage: {sys.argv[0]} <png> <width-in-mm> <gap-in-px>'
          f' <margin-in-mm> <smin> <smax> <fmin> <fmax> [-d]',
          file=sys.stderr)
    exit(1)
srcName = sys.argv[1]
...
def line(image, i, j, pixels):
    ...
    try:
        ...
        print(f'i={i} j={j}', file=sys.stderr)
        raise
    ...
    if lastVal == sat:
        ...
        if debug and lastVal >= 0 and lastVal < 255:
            print(f'{lastNum} v={lastVal}', file=sys.stderr)
        if lastVal < 255:
            if lastG0 != '':
                if speed > 0:
                    speed = 0
...
def convert(image):
    ...
    print(f'width:  {width:4d} px => {int(WIDTH):3d} mm', file=sys.stderr)
    print(f'height: {height:4d} px => {int(HEIGHT):3d} mm', file=sys.stderr)
